...
interface MartiniRound {
    roundNumber: number;
    difficulty: "easy" | "medium" | "hard";
    mode: "recipe" | "garnish" | "terminology" | "ingredient" | "technique";
    items: MartiniItem[];
}

const MARTINI_CATEGORY_LABELS: Record<string, { label: string; emoji: string; hint?: string }> = {
    "classic-dry": { label: "Classic Dry", emoji: "🍸", hint: "Gin-forward classics" },
    "classic-vodka": { label: "Vodka-Based", emoji: "🧊", hint: "Neutral spirit martinis" },
    "variation": { label: "Variation", emoji: "✨", hint: "Recognize non-classic styles" },
    "clarify-needed": { label: "Clarify Needed", emoji: "❓", hint: "Ask a follow-up question" },
    "ratio-adjustment": { label: "Ratio Adjustment", emoji: "⚖️", hint: "Vermouth or dilution preference" },
    "garnish-decision": { label: "Garnish Decision", emoji: "🍋", hint: "Garnish choice drives outcome" },
    "house-spec-check": { label: "House Spec Check", emoji: "🏛️", hint: "Confirm house build details" },
    "aromatic-structure": { label: "Aromatic Structure", emoji: "🌿", hint: "Aromatic backbone ingredient" },
    "texture-impact": { label: "Texture Impact", emoji: "🫧", hint: "Impacts smoothness/body" },
    "salinity-source": { label: "Salinity Source", emoji: "🫒", hint: "Brine or salt-driven modifier" },
    "fortified-wine": { label: "Fortified Wine", emoji: "🍷", hint: "Vermouth or fortified modifier" },
    "bitterness-accent": { label: "Bitterness Accent", emoji: "🍊", hint: "Bitters or bitter modifier" },
    "ratio-very-dry": { label: "Very Dry Ratio", emoji: "🧊", hint: "Very low vermouth" },
    "ratio-classic-dry": { label: "Classic Dry Ratio", emoji: "🍸", hint: "Standard dry martini spec" },
    "ratio-wet": { label: "Wet Ratio", emoji: "💧", hint: "Higher vermouth than classic dry" },
    "ratio-perfect": { label: "Perfect Ratio", emoji: "⚖️", hint: "Equal dry and sweet vermouth" },
    "martini-true": { label: "True Martini", emoji: "✅", hint: "Martini-family cocktails" },
    "martini-faux": { label: "Faux Martini", emoji: "🚫", hint: "Popular 'tinis' but not classic martinis" },
    "garnish-olive": { label: "Olive Garnish", emoji: "🫒", hint: "Savory garnish profile" },
    "garnish-twist": { label: "Lemon Twist", emoji: "🍋", hint: "Citrus aromatic garnish" },
    "garnish-onion": { label: "Gibson Onion", emoji: "🧅", hint: "Cocktail onion garnish" },
    terminology: { label: "Martini Terms", emoji: "📖", hint: "Service vocabulary" },
    "terminology-ratio": { label: "Ratio Terms", emoji: "⚖️", hint: "Vermouth-to-spirit language" },
    "terminology-style": { label: "Style Terms", emoji: "🗣️", hint: "Service and texture language" },
    "technique-stirred": { label: "Stirred", emoji: "🥄", hint: "Clear, silky texture" },
    "technique-shaken": { label: "Shaken", emoji: "🫧", hint: "Aerated and colder style" },
    "ingredient-base": { label: "Base Spirit", emoji: "🥃", hint: "Main spirit component" },
    "ingredient-modifier": { label: "Modifier", emoji: "🌿", hint: "Builds flavor profile" },
    "ingredient-garnish": { label: "Garnish Ingredient", emoji: "🍃", hint: "Final aromatic touch" },
};
...
export default function MatchingGame({ contentStr, activityId, assignmentId, vocabType }: Props) {
    ...
    // Martini sorting game
    if (gameMode === "martini-sorting") {
        return (
            <MartiniSortingUI
                contentStr={contentStr}
                activityId={activityId}
    ...
    return (
        <div className="fixed inset-0 bg-[var(--color-bg)] flex flex-col md:static md:max-w-4xl md:mx-auto md:px-3 md:py-4">
            ...
            {/* Header */}
            <div className="flex-shrink-0 bg-white border-b-2 md:border md:rounded-xl shadow-sm border-border p-3 md:p-4">
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="h-2.5 w-full bg-bg-gray rounded-full overflow-hidden">
                            <div
                            ...
                            className={`
                                ...
                                ${
                                    ...
                                    InteractionMode.WordSelected
                                        ...
                                        : "border-border-dark hover:border-blue-300"
                                }
                            `}
                        >
                                ...
                                className={`
                                    ...
                                    ${
                                        ...
                                        InteractionMode.WordSelected
                                            ...
                                            : "border-border-dark bg-bg-light hover:border-border-dark"
                                    }
                                `}
                            >
                            ...
                            {isGameComplete ? (
                                ...
                                <>
                                    ...
                                    <button
                                        ...
                                        className="inline-block px-6 py-3 bg-white text-emerald-600 font-bold rounded-lg hover:bg-bg-light active:scale-95 transition-[background-color,transform] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:ring-offset-2"
                                    >
                                        Next Round →
                                    </button>
                                </>
        ...
        <div className="max-w-4xl mx-auto p-4 md:p-6">
            <div className="bg-white border border-border rounded-xl shadow-sm p-4 mb-4">
                ...
                <div className="flex items-center gap-2">
                    <div className="h-2 flex-1 bg-bg-gray rounded-full overflow-hidden">
                        <div
                            className="h-full bg-[var(--color-primary)] transition-[width] duration-300"
                                ...
                                return (
                                    <button
                                        ...
                                        className={`
                                            ...
                                            ${selectedTermId === p.id ? "border-[var(--color-primary)] bg-[var(--color-primary)]/10" : "border-border hover:border-border-dark"}
                                        `}
                                        style={{
                                            borderStyle: 'solid',
                                ...
                                return (
                                    <button
                                        ...
                                        className={`
                                            ...
                                            ${matchedTermIds.has(p.id) ? "bg-green-50 border-green-400 text-green-900" : ""}
                                            ${wrongFlash === p.id ? "border-red-400 bg-red-50 animate-pulse" : "border-border hover:border-border-dark"}
                                        `}
                                        style={{
                                            borderStyle: 'solid',
                                ...
                                return (
                                    <button
                                        ...
                                        className={`
                                            ...
                                            ${matchedTermIds.has(p.id) ? "bg-green-50 border-green-400 text-green-900" : ""}
                                            ${selectedTermId === p.id ? "border-[var(--color-primary)] bg-[var(--color-primary)]/10" : "border-border-dark hover:border-border-dark"}
                                        `}
                                        style={{
                                            borderStyle: 'solid',
                                            backgroundColor: matchedTermIds.has(p.id) ? '#f0fdf4' : (selectedTermId === p.id ? 'rgba(176, 87, 64, 0.1)' : '#ffffff'),
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`
                                                w-full text-left px-4 py-4 min-h-[60px] rounded-lg border-2 transition-all touch-manipulation cursor-pointer text-base
                                                ${matchedTermIds.has(p.id) ? "bg-green-50 border-green-400 text-green-900" : ""}
                                                ${wrongFlash === p.id ? "border-red-400 bg-red-50 animate-pulse" : "border-border-dark hover:border-border-dark"}
                                            `}
                                            style={{
                                                borderStyle: 'solid',
                                                backgroundColor: matchedTermIds.has(p.id) ? '#f0fdf4' : (wrongFlash === p.id ? '#fef2f2' : '#ffffff'),
...
function getMartiniCategoryMeta(category: string): { label: string; emoji: string; hint?: string } {
    const normalized = category.trim().toLowerCase();
    if (MARTINI_CATEGORY_LABELS[normalized]) {
        return MARTINI_CATEGORY_LABELS[normalized];
    }
    if (normalized.startsWith("ingredient-")) {
        return { label: "Ingredient", emoji: "🍶", hint: "Ingredient classification" };
    }
    if (normalized.startsWith("garnish-")) {
        return { label: "Garnish", emoji: "🍋", hint: "Garnish style" };
    }
    if (normalized.startsWith("technique-")) {
        return { label: "Technique", emoji: "🛠️", hint: "Preparation method" };
    }
                    ...
                    <div className="w-full max-w-2xl">
                        <div
                            className={`
                                ...
                                ${gameState.bounceCard ? "animate-bounce-back border-border-dark" : "border-border-dark"}
                            `}
                        >
                                    ...
                                    <span key={blankIdx}>
                                        ...
                                        {blankIdx < arr.length - 1 && (
                                            gameState.filledBlanks[blankIdx] ? (
                                                ...
                                                // Future blank - dimmed
                                                <span className="inline-block min-w-[80px] md:min-w-[100px] border-b-4 border-dashed border-border-dark mx-1" />
                                            )
                                        )}
                                    </span>
                                ))}
                    ...
                    <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-2xl p-8 text-center shadow-xl w-full max-w-md">
                        ...
                        {isGameComplete ? (
                            ...
                            <>
                                ...
                                <button
                                    onClick={handleNextRound}
                                    className="inline-block px-6 py-3 bg-white text-emerald-600 font-bold rounded-lg hover:bg-bg-light active:scale-95 transition-colors"
                                >
                                    Next Round →
                                </button>
                            </>
...
function MartiniSortingUI({
    ...
    const rounds = useMemo(() => parseMartiniRounds(contentStr), [contentStr]);
    const [currentRoundIndex, setCurrentRoundIndex] = useState(0);
    const currentRound = rounds[currentRoundIndex];
    const shuffleSeed = useMemo(
        () => deriveShuffleSeed(currentRound?.roundNumber ?? 0, activityId),
    ...
    const overallCompletedItems = completedItemsBeforeCurrentRound + gameState.completedWords.size;
    const overallProgressPercent =
        totalItemsAcrossRounds > 0
            ? Math.round((overallCompletedItems / totalItemsAcrossRounds) * 100)
            : 0;
    ...
    const progress =
        shuffledItems.length > 0
            ? Math.round((gameState.completedWords.size / shuffledItems.length) * 100)
            : 0;
    const currentItem = shuffledItems[gameState.currentWordIndex];

    useEffect(() => {
        const styleId = "martini-game-styles";
        if (document.getElementById(styleId)) return;
    ...
    const handleDropZoneTap = (category: string) => {
        if (!currentItem) return;
        if (gameState.showExplanation) handleDismissExplanation();
        setInteractionMode(InteractionMode.Checking);
        checkAnswer(category);
    ...
    const checkAnswer = (selectedCategory: string) => {
        ...
        if (isCorrect) {
            ...
            setGameState((prev) => ({
                ...
                bounceWord: true,
                showExplanation: true,
                explanationText: currentItem.explanation,
            }));
    ...
    if (rounds.length === 0 || !currentRound) {
        return (
            <div className="max-w-6xl mx-auto p-8 text-center">
                <p className="text-text-muted">
                    No martini sorting items available.
                </p>
            </div>
        );
    }
    ...
    const difficultyStyle = difficultyColors[currentRound.difficulty];
    const modeLabel: Record<MartiniRound["mode"], { title: string; prompt: string }> = {
        recipe: { title: "Martini Recipes", prompt: "Sort each item into the right martini style" },
        garnish: { title: "Martini Garnishes", prompt: "Match each garnish to the right category" },
        terminology: { title: "Martini Terminology", prompt: "Sort each term by martini terminology groups" },
        ingredient: { title: "Martini Ingredients", prompt: "Sort each ingredient into the correct role" },
        technique: { title: "Martini Techniques", prompt: "Sort each example by preparation technique" },
    };
    const activeModeLabel = modeLabel[currentRound.mode];
    ...
    return (
        <div className="fixed inset-0 bg-[var(--color-bg)] flex flex-col md:static md:max-w-4xl md:mx-auto md:px-3 md:py-4">
            ...
            >
                {!isRoundComplete && currentItem && (
                    <div className="w-full max-w-md">
                        <div
                            ...
                            `}
                        >
                            <h3 className="text-2xl md:text-3xl font-bold text-text break-words">
                                {currentItem.item}
                            </h3>
                        </div>
                        <p className="text-center text-xs sm:text-sm text-text-muted mt-3">
                ...
                <div className="w-full max-w-3xl">
                    {!isRoundComplete ? (
                        <div
                            className={`${
                                categories.length > 2
                                    ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 w-full"
                                    : "grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 w-full"
                            }`}
                        >
                            {categories.map((category) => {
                                const meta = getMartiniCategoryMeta(category);
                                return (
                                    <div
                                        key={category}
                                        onDragOver={handleDropZoneDragOver}
                                        onDrop={(e) => handleDropZoneDrop(e, category)}
                                        onClick={() => handleDropZoneTap(category)}
                                        className={`
                                            relative p-6 md:p-8 rounded-xl border-4 border-dashed
                                            transition-[border-color,background-color] duration-200 cursor-pointer touch-manipulation focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:ring-offset-2
                                            min-h-[140px] flex flex-col items-center justify-center
                                            ${
                                                dropZoneFeedback?.category === category
                                                    ? dropZoneFeedback.isCorrect
                                                        ? "drop-zone-correct"
                                                        : "drop-zone-wrong"
                                                    : ""
                                    ...
                                    >
                                        ...
                                        <div className="font-bold text-text text-base md:text-lg text-center">
                                            {meta.label}
                                        </div>
                                        {meta.hint && (
                        ...
                        <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-2xl p-8 text-center shadow-xl">
                            ...
                            {isGameComplete ? (
                                ...
                                <>
                                    ...
                                    <p className="text-lg md:text-xl text-white/90 mb-2">
                                        You sorted all {shuffledItems.length} items!
                                    </p>
                                    <p className="text-sm text-white/80 mb-4">
                                        Ready for Round {currentRound.roundNumber + 1}?
                                    </p>
                ...
                {gameState.showExplanation && currentItem && (
                    <div className="w-full max-w-md">
                        <div className="bg-red-50 border-2 border-red-300 rounded-xl p-4 md:p-6 shadow-lg">
                            <div className="flex items-start gap-3 mb-3">
                                <span className="text-2xl">❌</span>
                            ...
                            <p className="text-sm md:text-base text-red-900 mb-3">
                                <strong>
                                    "{currentItem.item}" belongs in{" "}
                                    {getMartiniCategoryMeta(currentItem.category).label}
                                </strong>
                            </p>
                            <p className="text-sm text-red-800 mb-4 leading-relaxed">
                                {gameState.explanationText}
...
function detectMatchingGameMode(content: string): "vocab" | "countable" | "time-indicators" | "verb-sounds-right" | "martini-sorting" {
    ...
    for (const line of lines) {
        ...
        if (
            ...
            lower.includes("house-spec") ||
            lower.includes("aromatic-") ||
            lower.includes("texture-") ||
            lower.includes("salinity-") ||
            lower.includes("fortified-") ||
            lower.includes("bitterness-") ||
            lower.includes("martini-true") ||
            lower.includes("martini-faux")
        ) {
            hasMartiniLine = true;
        }
    ...
    if (hasRoundMarkers && hasMartiniLine) return "martini-sorting";
...
function parseMartiniRounds(content: string): MartiniRound[] {
    const rounds: MartiniRound[] = [];
    const roundBlocks = content.split(/\[ROUND\s*\d+[^\]]*\]/gi).filter((b) => b.trim());
    const roundHeaders = content.match(/\[ROUND\s*\d+[^\]]*\]/gi) || [];
    ...
    roundBlocks.forEach((block, index) => {
        ...
        const header = (roundHeaders[index] || "").toLowerCase();
        const difficulty: "easy" | "medium" | "hard" = header.includes("hard")
            ? "hard"
            : header.includes("medium")
            ? "medium"
            : "easy";

        const categoryText = items.map((item) => item.category.toLowerCase()).join(" ");
        let mode: MartiniRound["mode"] = "recipe";
        if (categoryText.includes("garnish")) mode = "garnish";
        else if (
            categoryText.includes("terminology") ||
            categoryText.includes("clarify") ||
            categoryText.includes("house-spec") ||
            categoryText.includes("ratio-")
        ) mode = "terminology";
        else if (categoryText.includes("ingredient")) mode = "ingredient";
        else if (categoryText.includes("technique")) mode = "technique";
